...
function FeatureBullet({ feature }: { feature: FeatureBullet }) {
  return (
    <div className="mb-5 flex items-start gap-3.5 last:mb-0">
      <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-lg ${feature.iconBg}`}>
        <Icon className={`h-[18px] w-[18px] ${feature.iconColor}`} name={feature.icon} />
      </div>
      <div>
        <h3 className="mb-1 text-[15px] font-semibold tracking-[-0.01em] text-[var(--ink)]">{feature.title}</h3>
  ...
  return (
    <section className="mt-[66px] bg-[var(--footer-bg)] px-10 pb-[100px] pt-[140px] text-center max-[860px]:px-6 max-[860px]:pb-16 max-[860px]:pt-[100px] max-[560px]:px-5">
      ...
      <p className="mx-auto mb-9 max-w-[560px] text-[clamp(15px,1.6vw,17px)] leading-[1.65] text-white/55">
        Stoxify gives SEBI-registered Research Analysts a complete platform to publish real-time trade ideas, grow a
        subscriber base, and build a scalable advisory practice.
      </p>
      <WaitlistForm />
      ...
      <div className="mx-auto grid max-w-[1100px] grid-cols-4 max-[860px]:grid-cols-2">
        {stats.map((stat, index) => (
          <div
            ...
            key={stat.label}
          >
            <div className="text-2xl font-bold leading-none tracking-[-0.5px] text-[var(--ink)]">{stat.value}</div>
      ...
      <div className="mx-auto max-w-[1100px]">
        <div className={revealClass()} data-reveal>
          ...
          <p className="mt-2.5 max-w-[560px] text-[clamp(15px,1.6vw,17px)] leading-[1.65] text-[var(--muted)]">
            Most RAs juggle 5+ tools. Stoxify replaces all of them - so you focus on research, not operations.
          </p>
        </div>
      ...
      <ul className="flex list-none flex-col gap-[11px]">
        {items.map((item) => (
          <li
            ...
            key={item}
          >
            <Icon className={`mt-0.5 h-4 w-4 shrink-0 ${dark ? "text-[#34d399]" : "text-[var(--red)]"}`} name={dark ? "check" : "x"} />
            <span>{item}</span>
          </li>
        ))}
      </ul>
      ...
      <div className="mx-auto max-w-[1100px]">
        <div className={revealClass(0, "mb-14 text-center")} data-reveal>
          ...
          <p className="mx-auto mt-3 max-w-[520px] text-[clamp(15px,1.6vw,17px)] leading-[1.65] text-[var(--muted)]">
            Stop juggling WhatsApp groups, spreadsheets, manual billing, and compliance paperwork. Stoxify handles it all.
          </p>
        </div>
        <div className={revealClass(0, "grid grid-cols-2 gap-12 max-[860px]:grid-cols-1")} data-reveal>
...
function ManageSection() {
  return (
    <section className="border-y border-[var(--line)] bg-[var(--surface)] px-10 py-[88px] max-[860px]:px-6 max-[860px]:py-16 max-[560px]:px-5 max-[560px]:py-14">
      <div className="mx-auto max-w-[1100px]">
        <div className={revealClass(0, "grid grid-cols-2 items-center gap-[72px] max-[860px]:grid-cols-1 max-[860px]:gap-10")} data-reveal>
          <div>
...
function DashboardMockup() {
  const rows = [
    { name: "RELIANCE BUY", range: `${"\u20B9"}2,800 -> ${"\u20B9"}2,940`, pnl: "+4.3%", status: "Active", active: true },
    { name: "INFY SELL", range: `${"\u20B9"}1,400 -> ${"\u20B9"}1,340`, pnl: "+4.1%", status: "Closed" },
    { name: "NIFTY CE BUY", range: `${"\u20B9"}220 -> ${"\u20B9"}280`, pnl: "+27%", status: "Closed" },
  ];
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-white/[0.08] bg-[#17191c] shadow-[0_16px_48px_rgba(0,0,0,0.12)]">
      <div className="flex items-center justify-between border-b border-white/[0.07] bg-[#1e2124] px-4 py-3">
        ...
        <button className="rounded bg-[var(--orange)] px-3 py-[5px] text-[11px] font-semibold text-white" type="button">
          + Publish
        </button>
      </div>
      <div className="grid grid-cols-3 gap-2 px-3.5 pt-3.5">
        <MetricCard color="text-[var(--orange)]" label="Monthly Revenue" sub="+18% this month" value={`${"\u20B9"}1.2L`} />
      ...
      <div className="p-3.5 pt-3">
        <div className="overflow-hidden rounded bg-white/[0.02]">
          ...
          {rows.map((row) => (
            <div className="grid grid-cols-[2fr_1fr_1fr] items-center border-b border-white/[0.03] px-3 py-2.5 last:border-b-0" key={row.name}>
              <div>
              ...
              <div>
                <span
                  className={[
                    ...
                    row.active ? "bg-[rgba(52,211,153,0.12)] text-[#34d399]" : "bg-white/[0.06] text-white/35",
                  ].join(" ")}
                >
                  {row.status}
                </span>
        ...
        <div className={revealClass(0, "grid grid-cols-2 items-center gap-[72px] max-[860px]:grid-cols-1 max-[860px]:gap-10")} data-reveal>
          <div>
            ...
            <p className="mb-7 text-[clamp(15px,1.6vw,17px)] leading-[1.65] text-[var(--muted)]">
              Our mobile-first publish interface is built for speed. Every field is optimised for the market - symbol
              search, type toggle, pre-filled exchanges.
            </p>
            <div className="flex flex-col gap-3.5 rounded-lg border border-[var(--line)] bg-white p-5">
              {timingRows.map((row) => (
                <div className="flex items-center gap-3 text-sm" key={row.label}>
                  ...
                  <span className="font-mono text-[15px] font-bold text-[var(--ink)]">{row.value}</span>
                </div>
              ))}
            </div>
          </div>
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-[var(--line)] bg-white shadow-[0_8px_24px_rgba(0,0,0,0.1)]">
      ...
      <div className="flex flex-col gap-[11px] p-[18px]">
        ...
        <PublishRow left={["Entry Price", `${"\u20B9"} 2,847`, "default"]} right={["Target", `${"\u20B9"} 2,940`, "buy"]} />
        <PublishRow left={["Stop Loss", `${"\u20B9"} 2,800`, "sell"]} right={["Risk:Reward", "1 : 2.0", "buy"]} />
        <div>
...
function FieldLabel({ children }: { children: React.ReactNode }) {
  return <div className="mb-[5px] text-[10px] font-bold uppercase tracking-[0.07em] text-[var(--muted-2)]">{children}</div>;
}

function PublishField({ field }: { field: [string, string, "brand" | "buy" | "sell" | "default"] }) {
  ...
  return (
    <div>
      ...
      <div className={`rounded border px-3 py-[9px] text-[13px] font-semibold ${classes[tone]}`}>{value}</div>
    </div>
  );
}
        ...
        <div className="mt-12 grid grid-cols-3 gap-5 max-[860px]:grid-cols-1">
          {howItWorks.map((item) => (
            ...
            >
              ...
              <h3 className="mb-2.5 text-base font-semibold tracking-[-0.01em] text-[var(--ink)]">{item.title}</h3>
              <p className="text-[13px] leading-[1.65] text-[var(--muted)]">{item.body}</p>
            </div>
          ))}
        </div>
        <div className={revealClass(0, "mt-9 grid grid-cols-3 gap-3.5 max-[860px]:grid-cols-1")} data-reveal>
          {chipItems.map((item) => (
            <div className="flex items-center gap-2.5 text-[13px] text-[var(--muted)]" key={item.text}>
      ...
      <div className="mx-auto grid max-w-[1100px] grid-cols-2 items-start gap-16 max-[860px]:grid-cols-1">
        <div className={revealClass()} data-reveal>
          ...
          <p className="text-[clamp(15px,1.6vw,17px)] leading-[1.65] text-white/45">
            Your revenue scales with your subscriber base. Stoxify handles every rupee - billing, renewals, and payouts.
          </p>
        </div>
        <div className={revealClass(2, "rounded-lg border border-white/10 bg-white/[0.05] p-7")} data-reveal>
          <div className="mb-5 text-[11px] font-semibold uppercase tracking-[0.08em] text-white/35">Sample Revenue Calculation</div>
          ...
          <div className="mt-4 flex items-center justify-between rounded border border-[rgba(249,115,22,0.25)] bg-[rgba(249,115,22,0.12)] px-5 py-4">
            ...
            <span className="text-[26px] font-bold tracking-[-1px] text-[var(--orange)]">~{"\u20B9"}2.7L</span>
          </div>
        </div>
      </div>
    </section>
...
function RevenueRow({ label, value, muted = false }: { label: string; value: string; muted?: boolean }) {
  return (
    <div className="flex items-center justify-between border-b border-white/[0.07] py-[13px] text-sm last:border-b-0">
      <span className="text-white/45">{label}</span>
      <span className={muted ? "font-semibold text-white/35" : "font-semibold text-white"}>{value}</span>
    </div>
  );
}
    ...
    <section className="px-10 py-[88px] max-[860px]:px-6 max-[860px]:py-16 max-[560px]:px-5 max-[560px]:py-14">
      <div className="mx-auto max-w-[1100px]">
        <div className={revealClass(0, "text-center")} data-reveal>
          ...
          <p className="mx-auto mt-3 max-w-[520px] text-[clamp(15px,1.6vw,17px)] leading-[1.65] text-[var(--muted)]">
            Every feature was designed with India&apos;s regulatory framework in mind - so you can focus on research, not paperwork.
          </p>
        </div>
        <div className="mt-12 grid grid-cols-3 gap-4 max-[860px]:grid-cols-1">
          {complianceCards.map((card) => (
            ...
            >
              ...
              <h3 className="mb-2 text-[15px] font-semibold tracking-[-0.01em] text-[var(--ink)]">{card.title}</h3>
...
function WaitlistCta() {
  return (
    <section className="bg-[var(--footer-bg)] px-10 py-24 text-center max-[860px]:px-6 max-[860px]:py-16 max-[560px]:px-5 max-[560px]:py-14" id="waitlist">
      <div className="mx-auto max-w-[1200px]">
        ...
        <p className={revealClass(2, "mx-auto mb-9 max-w-[640px] text-[clamp(15px,1.6vw,17px)] leading-[1.65] text-white/45")} data-reveal>
          We&apos;re onboarding our founding batch of SEBI-registered RAs. Get priority access, founding member benefits,
          and zero platform fee for your first 3 months.
        </p>
        <div className={revealClass(3)} data-reveal>
          <WaitlistForm />
        </div>
        <p className={revealClass(4, "mt-5 inline-flex items-center justify-center gap-2 text-[13px] text-white/35")} data-reveal>
        ...
        <div className={revealClass(4, "mt-3 flex flex-wrap justify-center gap-6")} data-reveal>
          {["Free to join", "Priority onboarding", "0% platform fee for first 3 months", "Founding member badge"].map((item) => (
            <span className="inline-flex items-center gap-1.5 text-xs text-white/35" key={item}>
              <Icon className="h-3.5 w-3.5 text-[#34d399]" name="check" />
              {item}
            </span>
    ...
    <footer className="bg-[var(--footer-bg)] px-10 pb-9 pt-16 max-[860px]:px-6 max-[860px]:pb-7 max-[860px]:pt-12 max-[560px]:px-5">
      <div className="mx-auto max-w-[1200px]">
        <div className="mb-14 grid grid-cols-[1.8fr_1fr_1fr_1fr] gap-12 max-[860px]:grid-cols-2 max-[860px]:gap-8 max-[560px]:grid-cols-1">
          <div>
            ...
            <p className="mb-5 text-sm leading-[1.7] text-white/40">
              India&apos;s first real-time trade idea marketplace - a complete business platform for SEBI-registered
              Research Analysts.
            </p>
          ...
          <FooterColumn links={["Join Waitlist", "RA Dashboard", "Revenue & Pricing", "Compliance Tools"]} title="For Research Analysts" />
          <FooterColumn links={["For Traders", "Home", "Blog", "Contact"]} title="Platform" />
          <FooterColumn links={["Privacy Policy", "Terms of Service", "SEBI Disclosures", "Grievance Redressal"]} title="Legal" />
        </div>
        <div className="flex flex-wrap items-center justify-between gap-3 border-t border-white/[0.08] pt-6 max-[860px]:flex-col max-[860px]:items-start max-[860px]:gap-2">
          <div className="text-[13px] text-white/30">&copy; 2026 Stoxify Technologies Pvt. Ltd. All rights reserved.</div>
        ...
        <p className="mt-5 border-t border-white/[0.06] pt-5 text-[11.5px] leading-[1.75] text-white/20">
          Stoxify is a SEBI-regulated technology platform. Only SEBI-registered Research Analysts (RAs) may publish trade
          ideas. All published ideas are the sole views of the respective RA. Past performance is not indicative of future
          results. Stoxify does not guarantee returns or accuracy of any trade idea.
        </p>
      </div>
    </footer>
  );
  ...
  return (
    <div>
      ...
      <ul className="flex list-none flex-col gap-[9px]">
        {links.map((link) => (
          <li key={link}>
            <Link className="text-sm text-white/55 transition-colors hover:text-white" href={link === "Join Waitlist" ? "#waitlist" : "#"}>
              {link}
            </Link>
          </li>
        ))}
