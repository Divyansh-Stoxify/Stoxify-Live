...
export default async function AnalystLandingPage({ params }: PageProps) {
  ...
  // Mock reviews
  const reviews = [
    { id: 1, rating: 5, text: "Excellent calls! The risk management is top notch and the analysis provided for each trade gives me a lot of confidence.", date: "2023-10-15" },
    { id: 2, rating: 4, text: "Very consistent performance. I've been a subscriber for 3 months and the returns have been stable. Sometimes entries are a bit fast.", date: "2023-11-02" },
    { id: 3, rating: 5, text: "Great transparent approach. All targets and stop losses are clearly mentioned and updated in real time.", date: "2023-11-20" }
  ];
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col transition-colors duration-300">
      <header className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 py-4 px-6 sticky top-0 z-40">
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          <Link href="/" className="font-bold text-xl text-slate-900 dark:text-white tracking-tight">Stoxify</Link>
          <ThemeToggle />
        </div>
      </header>
          ...
          <div className="flex flex-col md:flex-row items-center md:items-start gap-8">
            ...
            <div className="flex-1 text-center md:text-left">
              ...
              <div className="flex items-center justify-center md:justify-start gap-4">
                {profile.twitter_url && (
                  <a href={profile.twitter_url} target="_blank" rel="noopener noreferrer" className="text-slate-400 hover:text-[var(--brand)] dark:hover:text-[var(--brand)] transition-colors text-sm font-semibold">
                    X / Twitter
                  </a>
                )}
                {profile.linkedin_url && (
                  <a href={profile.linkedin_url} target="_blank" rel="noopener noreferrer" className="text-slate-400 hover:text-[var(--brand)] dark:hover:text-[var(--brand)] transition-colors text-sm font-semibold">
                    LinkedIn
                  </a>
                )}
                {profile.website && (
                  <a href={profile.website} target="_blank" rel="noopener noreferrer" className="text-slate-400 hover:text-[var(--brand)] dark:hover:text-[var(--brand)] transition-colors">
        ...
        <div className="max-w-5xl mx-auto px-6">
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 divide-x divide-slate-100 dark:divide-slate-800">
            ...
            <div className="flex flex-col items-center justify-center px-4">
              <div className="flex items-center gap-1.5 text-slate-500 dark:text-slate-400 mb-1">
                <Activity size={14} />
                <span className="text-xs font-semibold uppercase tracking-wider">Closed Trades</span>
              </div>
              <span className="text-2xl font-bold text-slate-900 dark:text-white">{stats.totalClosedTrades}</span>
            </div>
            <div className="flex flex-col items-center justify-center px-4">
              <div className="flex items-center gap-1.5 text-slate-500 dark:text-slate-400 mb-1">
                <Box size={14} />
              ...
              <span className="text-2xl font-bold text-slate-900 dark:text-white">{stats.activeBatches}</span>
            </div>
            <div className="flex flex-col items-center justify-center px-4">
              <div className="flex items-center gap-1.5 text-slate-500 dark:text-slate-400 mb-1">
                <Users size={14} />
      ...
      <div className="py-16">
        <div className="max-w-5xl mx-auto px-6">
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-8 text-center">Available Batches</h2>
          ...
          {plans.length === 0 ? (
            <div className="text-center py-12 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 transition-colors duration-300">
              ...
              <p className="text-slate-500 dark:text-slate-400 font-medium">No active batches available at the moment.</p>
            </div>
          ) : (
      ...
      <div className="bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 py-16 transition-colors duration-300">
        <div className="max-w-5xl mx-auto px-6">
          <div className="text-center mb-10">
            <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">Subscriber Reviews</h2>
            <div className="flex items-center justify-center gap-1 text-[var(--brand)]">
              {[1, 2, 3, 4, 5].map((i) => (
                <Star key={i} size={20} fill="currentColor" className={i === 5 ? "text-slate-200 dark:text-slate-700" : ""} />
              ))}
              <span className="ml-2 text-slate-700 dark:text-slate-300 font-bold text-lg">4.8</span>
              <span className="ml-1 text-slate-500 dark:text-slate-400 text-sm">({reviews.length} reviews)</span>
            </div>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {reviews.map((review) => (
              <div key={review.id} className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-6 border border-slate-100 dark:border-slate-800">
