...
import { useActiveTrades, useClosedTrades } from "@/hooks/use-analyst-dashboard";
...
function calculateRR(trade: Trade): string {
  if (!trade.exit_price || (!trade.stop_loss && !trade.stop_loss_price) || !trade.entry_price)
    return "—";
          ...
          <div className="text-right flex flex-col items-end">
            ...
            <span className="text-[11px] font-semibold text-gray-800">
              {trade.exit_timestamp
                ? new Date(trade.exit_timestamp).toLocaleString("en-US", {
                    month: "short",
                    day: "numeric",
                    year: "numeric",
                    hour: "numeric",
        ...
        <div className="mt-3 border border-gray-100 rounded-lg p-3">
          <div className="flex justify-between">
            ...
            <div className="flex flex-col items-end">
              ...
              <span className={`text-sm font-bold ${textColor}`}>{rr}</span>
            </div>
          </div>
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 shadow-sm mt-4 pb-6 overflow-hidden">
      ...
      <div className="px-5 space-y-6">
        ...
        <div className="space-y-4 border-b border-gray-100 pb-6">
          ...
          <div className="flex justify-between items-center text-[13px]">
            <span className="text-gray-700">
              Target {trade.targets && trade.targets.length > 0 && "(Max)"}
            </span>
        ...
        <div>
          <div className="text-[11px] font-bold text-gray-500 tracking-wider mb-4 uppercase">
...
function DetailedActiveCard({
  ...
  const estimatedGains = targetVal
    ? (Math.abs(targetVal - trade.entry_price) / trade.entry_price) * 100
    : 0;
  // Estimated risk (entry to SL)
  const estimatedRisk = stopLossVal
    ? (Math.abs(trade.entry_price - stopLossVal) / trade.entry_price) * 100
    : 0;
  // Live R/R
  ...
  const statusColor = isInBuyingRange
    ? "text-green-600"
    : isProfit
      ? "text-green-600"
      : "text-red-600";
    ...
    <div className="bg-white rounded-2xl border border-gray-200 shadow-sm mt-4 overflow-hidden">
      ...
      <div className="p-5 space-y-5">
        ...
        <div className="flex justify-between items-start">
          ...
          <div className="text-right">
            <div
              className={`text-xl font-extrabold text-gray-900 transition-colors duration-300 ${priceDirection === "up" ? "text-green-600" : priceDirection === "down" ? "text-red-600" : ""}`}
            >
              ₹{liveLtp.toFixed(2)}
            </div>
            <div
              className={`text-sm font-bold mt-0.5 ${isProfit ? "text-green-600" : "text-red-500"}`}
            >
              {isProfit ? "+" : "-"}₹{Math.abs(livePnlPerUnit).toFixed(2)} ({isProfit ? "+" : ""}
        ...
        <div className="border border-gray-200 rounded-xl p-4 space-y-3">
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="text-right">
              ...
              <div
                className={`text-lg font-bold ${liveRR >= 0 ? "text-green-600" : "text-red-600"}`}
              >
                {liveRR >= 0 ? "+" : ""}
                {liveRR.toFixed(2)}x
              </div>
        ...
        <div className="border border-gray-200 rounded-xl p-4 flex justify-between">
          ...
          <div className="text-center flex-1 flex flex-col justify-start">
            <div className="text-[13px] font-medium text-gray-500 mb-1">
              Target{trade.targets && trade.targets.length > 1 ? "s" : ""}
            </div>
          ...
          <div className="text-center flex-1 flex flex-col justify-start">
            ...
            <div className="font-bold text-base text-red-600">
              {stopLossVal ? `₹${stopLossVal.toFixed(2)}` : "—"}
            </div>
          </div>
        </div>
        ...
        <div className="border border-gray-200 rounded-xl p-4 space-y-4">
          <div className="flex justify-between items-center text-[13px]">
            ...
            <span className="font-semibold text-gray-900">
              {formatDate(trade.nse_timestamp ?? trade.created_at)}
            </span>
          </div>
          <div className="h-px bg-gray-100" />
          <div className="flex justify-between items-center text-[13px]">
                    ...
                    {Object.entries(mod.fields_changed).map(([key, changes]) => {
                      ...
                      return (
                        <div key={key}>
                          ...
                          : ₹{String(changes.old)} →{" "}
                          <span className="font-bold text-gray-900">₹{String(changes.new)}</span>
                        </div>
                      );
                    })}
                  </div>
...
function TradeCard({
  ...
  liveLtpProp?: number;
  onAutoClose?: () => void;
  onOptimisticClose?: (tradeId: string) => void;
}) {
  ...
  const isTargetHit =
    targetVal !== undefined && (!isShort ? liveLtp >= targetVal : liveLtp <= targetVal);
  const isSLHit =
    stopLossVal !== undefined && (!isShort ? liveLtp <= stopLossVal : liveLtp >= stopLossVal);
  ...
  const entryPct =
    targetVal && stopLossVal && range !== 0
      ? Math.min(
          100,
          Math.max(
            0,
            (!isShort
              ? (trade.entry_price - stopLossVal) / range
              : (stopLossVal - trade.entry_price) / range) * 100
          )
        )
      : 50;
  const livePct =
    targetVal && stopLossVal && range !== 0
      ? Math.min(
          ...
          Math.max(
            0,
            (!isShort ? (liveLtp - stopLossVal) / range : (stopLossVal - liveLtp) / range) * 100
          )
        )
      : 50;
      ...
      <div className="p-5 flex-1 cursor-pointer" onClick={() => setShowDetailsModal(true)}>
        ...
        <div className="flex justify-between items-start mb-10">
          ...
          <div className="text-right">
            ...
            <div
              className={`text-sm font-bold mt-1 ${livePnlPerUnit >= 0 ? "text-green-600" : "text-red-500"}`}
            >
              <span>
                {livePnlPerUnit >= 0 ? "+" : "-"}₹{Math.abs(livePnlPerUnit).toFixed(2)} (
                {livePnlPerUnit >= 0 ? "+" : ""}
                {livePnlPct.toFixed(2)}%)
              </span>
            </div>
        ...
        <div className="mb-10 mt-12 mx-4">
          ...
          <div className="relative">
            ...
            <div
              className="absolute -top-7 -translate-x-1/2 whitespace-nowrap text-[11px] font-semibold text-gray-500"
              style={{ left: `${entryPct}%` }}
            >
            ...
            <div
              className="absolute -top-12 -translate-x-1/2 whitespace-nowrap text-[12.5px] font-bold text-blue-600 z-10 bg-white/90 backdrop-blur-sm px-1.5 py-0.5 rounded-md shadow-sm border border-blue-100"
              style={{ left: `${livePct}%` }}
            >
            ...
            <div className="relative h-10 text-[11px] font-bold">
              {stopLossVal && (
                <div
                  className="absolute top-2 whitespace-nowrap text-red-500"
                  style={{ left: "0%" }}
                >
              ...
              {trade.targets && trade.targets.length > 1
                      ...
                      targetVal && stopLossVal && range !== 0
                        ? Math.min(
                            ...
                            Math.max(
                              0,
                              (!isShort
                                ? (t.target_price - stopLossVal) / range
                                : (stopLossVal - t.target_price) / range) * 100
                            )
                          )
                        : 50;
                    return (
                ...
                : targetVal && (
                    <div
                      className="absolute top-2 -translate-x-full whitespace-nowrap text-[#5b982c]"
                      style={{ left: "100%" }}
                    >
        ...
        <div className="bg-[#f8faf9] rounded-xl p-4 flex justify-between border border-gray-100 mb-4 shadow-sm">
          <div>
            ...
            <div className="text-green-600 font-bold text-base">
              {targetVal
                ? `+${((Math.abs(targetVal - trade.entry_price) / trade.entry_price) * 100).toFixed(2)}%`
                : "—"}
            </div>
          </div>
          <div className="text-right">
            ...
            <div
              className={`font-bold text-base ${isTargetHit ? "text-green-600" : isSLHit ? "text-red-600" : Math.abs(liveLtp - trade.entry_price) / trade.entry_price < 0.005 ? "text-green-600" : livePnlPerUnit >= 0 ? "text-green-600" : "text-red-600"}`}
            >
              {isTargetHit
                ? "Target Hit"
                : isSLHit
                  ? "SL Hit"
                  : Math.abs(liveLtp - trade.entry_price) / trade.entry_price < 0.005
                    ? "In Buying Range"
                    : livePnlPerUnit >= 0
                      ? "In Profit"
                      : "In Loss"}
      ...
      <div className="flex items-center gap-4 border-t border-gray-100 bg-gray-50/50 px-5 py-4 mt-auto max-[860px]:flex-col max-[860px]:items-stretch">
        <div className="flex flex-1 items-center gap-3 text-sm min-w-0">
          <button
            ...
            title={
              isNotePublic
                ? "Public note - visible to subscribers"
                : "Private note - hidden from subscribers"
            }
          >
...
export default function LiveTradesPage() {
  ...
  const {
    ...
    tradeClosedEvent,
    tradeModifiedEvent,
  } = useWebSocket(activeTrades.map((t) => t.symbol));
