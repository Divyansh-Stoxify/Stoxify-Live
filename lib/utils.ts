...
export function cleanErrorMessage(data: any, defaultMsg: string = 'An unexpected error occurred. Please try again.'): string {
  ...
  if (code === 'INSUFFICIENT_POWER') {
    ...
    if (stateMatch) {
      ...
      const stateDescription = stateMap[rawState] || `in state ${rawState.toLowerCase().replace(/_/g, ' ')}`;
  ...
  if (fallbackMsg) {
    const technicalJargon = [
      /database/i, /mongo/i, /sql/i, /query/i, /connection/i, /syntax/i,
      /typeerror/i, /undefined/i, /null/i, /failed to serialize/i,
      /validation/i, /cast to/i, /duplicate key/i, /internal server/i,
      /uncaught/i, /referenceerror/i
    ];
