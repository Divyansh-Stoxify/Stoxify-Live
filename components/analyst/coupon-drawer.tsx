            ...
            <div className="flex-1 overflow-y-auto px-6 pb-6 flex flex-col gap-3">
              {isLoading ? (
                <div className="flex justify-center p-8">
                  <Icon className="h-6 w-6 animate-spin text-slate-300" name="loader" />
                </div>
              ) : coupons.length === 0 ? (
                <div className="text-center p-8 text-slate-400 text-[14px]">
                ...
                coupons.map((c) => (
                  <div
                    key={c._id}
                    className="border border-slate-200 rounded-xl p-4 flex justify-between items-center bg-white shadow-sm"
                  >
                    <div>
                      ...
                      <div className="text-[12px] text-slate-500 mt-1">
                        Used: {c.quantity_used}{" "}
                        {c.quantity_total ? `/ ${c.quantity_total}` : "(Unlimited)"}
                      </div>
                    </div>
                    <button
          ...
          <>
            ...
            <div className="p-6 flex flex-col gap-4">
              <button
                ...
                className="p-5 border border-slate-200 rounded-2xl hover:border-indigo-400 hover:shadow-md text-left transition-all bg-white group"
              >
                <div className="font-bold text-slate-900 text-[15px] group-hover:text-indigo-600 transition-colors">
                  Percentage Discount
                </div>
                <div className="text-slate-500 text-[13px] mt-1">
                  e.g. 20% off all subscriptions
                </div>
              </button>
              <button
                onClick={() => {
                ...
                className="p-5 border border-slate-200 rounded-2xl hover:border-emerald-400 hover:shadow-md text-left transition-all bg-white group"
              >
            ...
            <div className="flex items-center px-6 py-4 border-b border-slate-100 bg-slate-50 gap-3">
              ...
              <h2 className="text-[16px] font-extrabold flex items-center gap-2">
                <Icon
                  ...
                  name={type === "PERCENTAGE" ? "badge" : "rupee"}
                />
              ...
              <form id="coupon-form" onSubmit={handleCreate} className="flex flex-col gap-6">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <label
                      className={`border rounded-lg p-3 cursor-pointer text-center text-[13px] font-bold transition-all ${availability === "EVERYONE" ? "border-slate-900 bg-slate-900 text-white" : "border-slate-200 text-slate-600 hover:bg-slate-50"}`}
                    >
                      <input
                        type="radio"
                        className="hidden"
                        checked={availability === "EVERYONE"}
                        onChange={() => setAvailability("EVERYONE")}
                      />
                      All Plans
                    </label>
                    <label
                      className={`border rounded-lg p-3 cursor-pointer text-center text-[13px] font-bold transition-all ${availability === "SPECIFIC" ? "border-slate-900 bg-slate-900 text-white" : "border-slate-200 text-slate-600 hover:bg-slate-50"}`}
                    >
                      <input
                        type="radio"
                        className="hidden"
                        checked={availability === "SPECIFIC"}
                        onChange={() => setAvailability("SPECIFIC")}
                      />
                      This Plan Only
                    </label>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="flex flex-col gap-2">
                    ...
                    <input
                      ...
                      className="w-full rounded-lg border border-slate-200 px-3 py-2 text-[13px] text-slate-900 outline-none focus:ring-2 focus:border-slate-900"
                    />
                  </div>
                  <div className="flex flex-col gap-2">
