...
export default function WithdrawalHistoryTable() {
  // Mock data
  const withdrawals = [
    { id: 'w1', date: '2026-06-28', amount: 5000, bankAccount: 'XXXXXXXXX1234', utr: 'IMPS123456789', status: 'processed' },
    { id: 'w2', date: '2026-06-20', amount: 7500, bankAccount: 'XXXXXXXXX1234', utr: '-', status: 'processing' },
    { id: 'w3', date: '2026-06-15', amount: 6000, bankAccount: 'XXXXXXXXX1234', utr: '-', status: 'failed' },
  ];

  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'processed':
        return <span className="px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400">Processed</span>;
      case 'processing':
        return <span className="px-2.5 py-0.5 rounded-full text-xs font-medium bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-400">Processing</span>;
      case 'failed':
        return <span className="px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400">Failed</span>;
      default:
